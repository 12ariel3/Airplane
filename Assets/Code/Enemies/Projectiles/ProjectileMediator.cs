...
namespace Assets.Code.Enemies.Projectiles
{
    public class ProjectileMediator : RecyclableObject, Projectile, EventObserver
    {
        ...
        public void Configure(ProjectileConfiguration configuration)
        {
            ...
            _turnSpeed = configuration.TurnSpeed;
            _movementController.Configure(configuration.DirectionPosition, _speed, _turnSpeed);
        ...
        private void Update()
        {
            if (_startDestroying && !_gameOver && !_victory)
            {
        ...
        public void Process(EventData eventData)
        {
            ...
            {
                _gameOver = false;
                Debug.Log("ProjectileMediator after Event ContinueBattleAfterAds.... llego el lechero!!");
            ...
            if (eventData.EventId == EventIds.LastProjectileAlive)
            {
                if (!_deathOnce && !_startDestroying)
                {
                    _counter = 8f;
                    _startDestroying = true;
                }
            ...
            if (eventData.EventId == EventIds.PlayerSpamedAndSendHisTransform)
            {
                ...
                _movementController.Follow(playerSpamedAndSendHisTransformEventData.PlayerTransform);
            }
        }
    }
}